...
contract AletheoTrustMinimizedProxy{ // THE CODE FITS ON THE SCREEN UNBELIAVABLE LETS STOP ENDLESS SCROLLING UP AND DOWN
	...
	function _trustMinimized() internal view returns (uint tm) {
		assembly { tm := sload(TRUST_MINIMIZED_SLOT) }
	}
	...
	function upgrade() external ifAdmin {
		...
		address logic;
		assembly {logic := sload(NEXT_LOGIC_SLOT) sstore(LOGIC_SLOT,logic)}
		emit Upgraded(logic);
	}
	...
	function cancelUpgrade() external ifAdmin {
		address logic;
		assembly {logic := sload(LOGIC_SLOT)sstore(NEXT_LOGIC_SLOT, logic)}
	...
	function prolongLock(uint b) external ifAdmin {
		...
		emit ProposingUpgradesRestrictedUntil(b,b+1296000);
	}
