...
interface I{function getRewards(address a,uint rewToClaim) external returns(bool);function balanceOf(address) external view returns(uint); function genesisBlock() external view returns(uint);}
...
contract PosterRewards {
	...
	function init() public {
		...
		oracle = 0x5C8403A2617aca5C86946E32E14148776E37f72A;//to change, after giving our oracle different address
	}
	...
	function getRewards(uint amount)external{
		...
		if (allowed>=amount){
			...
			bool success = I(0x05658a207a56AA2d6b2821883D373f59Ac6A2fC3).getRewards(a, amount); require(success == true);
		}
	}

	function setOracle(address a) {	require(msg.sender==deployer); oracle = a; }

	function getOracleGas(uint amount){
		uint genesisBlock = I(0x31A188024FcD6E462aBF157F879Fb7da37D6AB2f).genesisBlock();
		...
		if (allowed>=amount){
			bool success = I(0x05658a207a56AA2d6b2821883D373f59Ac6A2fC3).getRewards(oracle, amount); require(success == true);
		}
	}
}