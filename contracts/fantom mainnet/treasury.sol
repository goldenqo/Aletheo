...
contract Treasury {
	...
	struct Beneficiary {uint128 amount; uint128 emission; uint lastClaim;}
	...
	struct Refund {uint128 amount; uint128 lastClaim; uint emission;}
	...
	function addAirdropBulk(address[] memory r,uint[] memory amounts) external {
		...
		for(uint i = 0;i<r.length;i++) {
			if(airdrops[r[i]].amount==0){
				totalAirdrops+=1; airdrops[r[i]].amount = uint128(amounts[i]); if(airdrops[r[i]].lastClaim==0){ airdrops[r[i]].lastClaim=uint128(block.number); }
			}
		}
	}
