...
interface I{
	function transfer(address to, uint value) external returns(bool); function claimRefunds() external view returns(uint); function claimPosterRewards() external returns(uint);
	function claimAirdrop() external returns(uint); function genesisBlock() external view returns(uint); function balanceOf(address) external view returns(uint);
}

contract TreasurySafeRelay {
	address private _governance;
	struct Refund {uint128 lastClaim; uint128 emission;}
...
/*0x543af6FF1c1195e56bdB91B4422A1bbd0dC4A945*/	addAirdrop(0x543af6FF1c1195e56bdB91B4422A1bbd0dC4A945,44539335228e9);
	...
	function _getRate() internal view returns(uint){uint rate = 31e12*99; uint quarter = block.number/28e6;if (quarter>0) {for (uint i=0;i<quarter;i++) {rate=rate*4/5;}}return rate;}
	
	function addRefund(address r,uint emission) public {
		require(msg.sender == _governance);
		uint lc = refunds[r].lastClaim;
		if (lc == 0) {refunds[r].lastClaim = uint128(block.number);}
		refunds[r].emission = uint128(emission);
		totalEmission+=emission;
	}
	
	function getRefundRewards()external returns(uint){
		uint genesisBlock = I(0x8bd7AbF86696f1922BeeC10Cccda9a79822f03fd).genesisBlock();
		I(0x75b13c7CDB6C957526C0741708f04B35dFc812a1).claimRefunds();
		uint lastClaim = refunds[msg.sender].lastClaim; refunds[msg.sender].lastClaim=uint128(block.number);
		require(genesisBlock!=0&&block.number>lastClaim&&block.number>genesisBlock);
		if(genesisBlock>lastClaim){lastClaim=genesisBlock;}
		uint rate=_getRate();
		uint toClaim = (block.number-lastClaim)*rate*refunds[msg.sender].emission/totalEmission;
		if(toClaim>I(0xaBFAD5E1a12C8BFCadff3Bb47A526510817eeed1).balanceOf(address(this))){toClaim=I(0xaBFAD5E1a12C8BFCadff3Bb47A526510817eeed1).balanceOf(address(this));}
		I(0xaBFAD5E1a12C8BFCadff3Bb47A526510817eeed1).transfer(msg.sender,toClaim);
		return toClaim;
	}
	...
	function addAirdropBulk(address[] memory r,uint[] memory amounts) external {
		...
		for(uint i = 0;i<r.length;i++) {
			if(airdrops[r[i]].amount==0){
				totalAirdrops+=1; airdrops[r[i]].amount = uint128(amounts[i]);
	...
	function addAirdrop(address r,uint amount) public {
		require(msg.sender == _governance);
		if(airdrops[r].amount==0){totalAirdrops+=1;}
		airdrops[r].amount += uint128(amount);
		if(airdrops[r].lastClaim==0){ airdrops[r].lastClaim=uint128(block.number); }
	}

	function claimAirdrop()external {//genesis posters too
		uint genesisBlock = I(0x8bd7AbF86696f1922BeeC10Cccda9a79822f03fd).genesisBlock();
		I(0x75b13c7CDB6C957526C0741708f04B35dFc812a1).claimAirdrop();
	...
	function claimPosterRewards()external {
		...
		uint rate=31e12*99;rate*=2; uint toClaim =(block.number-lastClaim)*rate/totalPosters;
