...
describe('FOUNDINGEVENT', function () {
  ...
  describe('init()', function () {
    it('Initializes state variables correctly', async function () {
      ...
      expect(await foundingEvent.deployer(), 'unexpected deployer').to.equal(accounts[0].address);
      expect(await foundingEvent.letToken(), 'unexpected letToken').to.equal(eerc20.address);
      expect(await foundingEvent.WBNB(), 'unexpected WBNB').to.equal(wbnb.address);
      expect(await foundingEvent.BUSD(), 'unexpected BUSD').to.equal(busd.address);
      expect(await foundingEvent.router(), 'unexpected router').to.equal(router.address);
      expect(await foundingEvent.factory(), 'unexpected factory').to.equal(factory.address);
