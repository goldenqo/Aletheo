...
describe('EERC20', function () {
  ...
  describe('init()', function () {
    it('Initializes correctly', async function () {
      ...
      expect(await eerc20.liquidityManager(), 'unexpected liquidityManager value').to.equal(accounts[2].address);
      expect(await eerc20.treasury(), 'unexpected treasury value').to.equal(accounts[3].address);
      expect(await eerc20.foundingEvent(), 'unexpected foundingEvent value').to.equal(accounts[4].address);
      expect(await eerc20.governance(), 'unexpected governance value').to.equal(accounts[0].address);
      ...
      expect(await eerc20.balanceOf(accounts[4].address), 'unexpected foundingEvent balance').to.equal(
  ...
  describe('setLiquidityManager()', function () {
    it('Sets liquidityManager if called by governance', async function () {
    ...
    it('Fails to set liquidityManager if called by not governance', async function () {
  ...
  describe('transferFrom()', function () {
    ...
    it('Sets correct balances for normal accounts if sellTax is not 0', async function () {
      ...
      const arg = 5,
        sellTax = 20;
      const { initialBalance, initialRecipientBalance } = await transferFromHelper(
        accounts[1],
        accounts[0].address,
        accounts[1].address,
        sellTax,
